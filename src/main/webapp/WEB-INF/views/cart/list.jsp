...
function batchDelete() {
	...
	location = "../cart/delete/" + cartItemIdArray;
}
...
<c:forEach items="${cartItemList }" var="cartItem">
	<tr align="center">
		...
		<td align="left" width="70px">
			<a class="linkImage" href="<c:url value='../flower/loadDesc/${cartItem.flower.fid }'/>"><img border="0" width="54" align="top" src="${pageContext.request.contextPath}/${cartItem.flower.image_b }"/></a>
		</td>
		<td align="left" width="400px">
		    <a href="<c:url value='../flower/loadDesc/${cartItem.flower.fid }'/>"><span>${cartItem.flower.fname }</span></a>
		</td>
		<td>
			<span>&yen;<span class="currPrice">${cartItem.flower.currPrice }
			</span></span>
		</td>
		<td>
			<a class="jia" id="${cartItem.cartItemId }"></a>
				<input class="quantity" readonly="readonly" id="${cartItem.cartItemId }Quantity" type="text" value="${cartItem.quantity }"/>
			<a class="jian" id="${cartItem.cartItemId }"></a>
		</td>
		<td width="100px">
			<span class="price_n">&yen;<span class="subTotal" id="${cartItem.cartItemId }Subtotal">${cartItem.subtotal }</span></span>
		</td>
		<td>
			<a href="<c:url value='../cart/delete/${cartItem.cartItemId }'/>">删除</a>
		</td>
	</tr>
</c:forEach>
	...
	<form id="jieSuanForm" action="<c:url value='../cart/loadjiesuan'/>" method="post">
